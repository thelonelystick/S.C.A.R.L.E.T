    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-footer">
          ...
          <button className="profile-row" onClick={() => setActiveView('settings')} aria-label="Open profile settings"><div className="profile-avatar">{user?.email?.slice(0, 1).toUpperCase() ?? 'A'}</div><div><p className="profile-name">{user?.user_metadata?.full_name ?? user?.email?.split('@')[0] ?? 'Alex'}</p><p className="profile-status">{user ? 'Google account' : 'Local workspace'}</p></div><span className="status-dot" /></button>
        </div>
      </aside>
